...
<p style="margin:6px 0 0;color:rgba(255,255,255,0.8);font-size:12px;letter-spacing:1.5px;text-transform:uppercase;">Luxury Skincare · Canada</p>
</td></tr>
...
`, `New order #${order_number} from ${name}`);
    ...
    if (phone) {
        const smsMessage = trackingNumber
            ...
            : `Hi ${name}, your order #${order_number || id} at TOUCHEEGLOW is confirmed! Track here: ${trackingUrl}${shippingNotesSms}`;
...
<p style="color:#374151;font-size:14px;line-height:1.7;margin:16px 0;">Thank you for joining the ${BRAND.name} family. We source premium luxury skincare products formulated to help you achieve a flawless, healthy appearance.</p>
...
`, `Welcome to ${BRAND.name}, ${firstName}!`)
    ...
    if (phone) {
        await sendSMS({
            to: phone,
            message: `Welcome ${firstName}! Thanks for joining TOUCHEEGLOW.`
        });
    }
}
